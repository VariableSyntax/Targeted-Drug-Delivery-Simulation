...
function [fval] = domain_1D_Sph(t,y,P1,P2,P3)
   
    % Define the number of spatial 'slices'     
    R = (numel(y)-1)/2+1; % step size = 1/R; 
    ...
    for i = 2:R % Iterate throughout all slices between the center and edge 
        ...
        if i > 2 && i < R     % If slice i is between the center and edge 
        % Discretized 2nd order derivative (gradient) of [Drug] in 1D space 
        % (central difference formula)
        d2Drug_dr2(i) = (2/i)*(Drug(i+1)-Drug(i-1))/...
                (2/R)+(Drug(i+1)-2*Drug(i)+Drug(i-1))/((1/R)^2);
        ...
        elseif i == 2 % for slice immediately after the center 
            % (i = 1 is center)
            d2Drug_dr2(i) = (2/i)*(Drug(i+1)-Drug(i))/...
                (1/R)+(Drug(i+1)-Drug(i))/((1/R)^2);
        
        % Change in [Drug] in the hydrogel at spatial slice (i) 
            dC_dt(i) = P2*d2Drug_dr2(i)-dDrugCD_dt(i);
          
        elseif i == R % slice on the edge (i = N in Fu paper)
            d2Drug_dr2(i)= (2/i)*(Drug(i)-Drug(i-1))/...
                (1/R)+(-2*Drug(i)+Drug(i-1))/((1/R)^2);
        % Change in [Drug] in the hydrogel at spatial slice (i) 
            dC_dt(i) = P2*d2Drug_dr2(i)-dDrugCD_dt(i);
