...
function [cumul_release, output] = profile_1D_Cyl(DrugSol,tSol,DrugCDSol,...
                    Drug_0,R0,h,r,Volume,Figure_1,Figure_2,Figure_3,...
...
for i = 2:size(frac_release,1) % Iterate throughout all timepoints of the 
   ...
   % specified thresholds (tolerances)
    if abs(release(i)-release(i-1))/(tSol(i)-tSol(i-1)) <= 0.0005
       % slope is approx zero
       tStop(count2) = tSol(i-1);
       count2 = count2 + 1;
   end 
end 
tBurst = tBurst(1); % Time to Reach 90% Drug Release is the 1st value 
                    % of tBurst
...
if Figure_1 ~= 0    % If Figure 1 is selected by the user
    ...
    legend('Model Output','90% Drug Release','Initial Drug Conc.','Location','Southeast')
...
if Figure_5 ~= 0    % If Figure 4 is selected by the user
    Fig5 = figure;
...
if Animation ~=0   % If the user wants an Animation
    ...
    for k = 1:numel(tSol)    % Iterate through all time points in model
        ...
        count = 1;      % 2nd counter
        for i = 2:R0    % Iterate through all spatial slices (r-dir)
            z1 = h/2*10^3;    % Convert the height to mm
            rho = r*i/R0*10^3; % Convert the radius to mm
            [x,y,z] = cylinder(rho);    % Make a cylinder of radius(rho)
            z(2,:) = z1;    % Offset by previous height
            [~,b] = size(x);
        ...
        % Preallocate space for reflection z-axis
        [v,d] =size(x_vec);
        x_vec2 = zeros(v*2,d);
        y_vec2 = zeros(v*2,d);
        z_vec2 = zeros(v*2,d);
        ...
        % Reflect previous cylinder along the x-axis
    ...
    if filename == 0   % If the filename is empty use default filename
        filename = '1D_Cylinder_Hydrogel_Animation.mp4';
    end
