...
function [cumul_release, output] = profile_1D_Slab(DrugSol,tSol,...
                                DrugCDSol,Drug_0,N0,h,r,Volume,...
                                Figure_1,Figure_2,Figure_3,Figure_4,...
                                Figure_5,Animation,UploadData)
...
for i = 2:size(frac_release,1) % Iterate throughout all timepoints of the 
   ...
   if abs(release(i-1)-release(i))/abs(tSol(i)-tSol(i-1)) <= 0.0005           
...
if Figure_5 ~= 0    % If Figure 5 is selected by the user
    Fig5 = figure;
    xaxis = tSol;   % Time is the x-axis
    yaxis = linspace(0,h/2*10^3,N0+1); % Space (z-dir) is the y-axis
...
if tStop == 0 || tStop < tBurst    % If Equilibrium is NOT reached
    ...
    tStop_str = strjoin({'Time to Reach Equilibrium = ',num2str(tStop),...
        ' hr'}); 
end
...
if Animation ~= 0   % If the user wants an Animation
    ...
    if isvalid(settings)    % If the animation parameters are changed
    ...
    for k = 1:numel(tSol)    % Iterate through all time points in model
        ...
        z_prev = 0;     % Initial z-point for each figure frame
        count = 1;      % 2nd counter
        for i = 2:N0    % Iterate through all spatial slices (z-dir)
            z1 = h/2*i/N0*10^3;    % Convert the height to mm
            for j = 2:N0           % Iterate through all slices 
                rho = r*j/N0*10^3; % Convert the radius to mm
                [x,y,z] = cylinder(rho);   % Make a cylinder of radius(rho)
                % Specify the start/end heights
                z(1,:) = z(1,:) + z_prev;  
                % Offset by previous height
                z(2,:) = z1;
                z_prev = z1;
                [~,b] = size(x);    
                drug = zeros(2,b);  % Preallocate space 
                drug(1:2,:) = DrugSol(k,i); % Extract [Drug] values
                % Extract corresponing coordinates (x,y,z) of cylinder
                x_vec(count:count+1,:) = x; 
                y_vec(count:count+1,:) = y;
                z_vec(count:count+1,:) = z;
                % Extract corresponding [Drug] values for each slice
                drug_vec(count:count+1,:) = drug;
                count = count + 2; % Iterate through 2nd counter
            end 
        end 
        
        % Preallocate space for reflection x-axis
        [v,d] =size(x_vec);    
        x_vec2 = zeros(v*2,d);
        y_vec2 = zeros(v*2,d);
        z_vec2 = zeros(v*2,d);
        ...
        % Reflect previous cylinder along the z-axis
    ...
    if filename == 0   % If the filename is empty use default filename
        filename = '1D_Slab_Hydrogel_Animation.mp4';
    end
