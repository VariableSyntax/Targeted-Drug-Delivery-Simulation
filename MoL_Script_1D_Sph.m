...
function [tSol,cumul_release,output] = MoL_Script_1D_Sph(r,Drug_0,CD_0,...
    K,k2,D,hrs,res,Figure_1,Figure_2,Figure_3,Figure_4,Figure_5,Animation,...
    UploadData)
%% Initialization

% Define hydrogel dimensions
A = 4*r.^2.*pi; % Hydrogel cross-sectional area (m^2)
Volume = 4/3.*pi.*r.^3; % Hydrogel volume (m^3) 
...
[tauSol, ODESol] = ode15s(@domain_1D_Sph,tau_Span,Drug0,[],P1,P2,P3); 
...
[cumul_release, output ]= profile_1D_Sph(DrugSol,tSol,DrugCDSol,Drug_0,...
                    R0,r,Volume,Figure_1,Figure_2,Figure_3,...
                    Figure_4,Figure_5,Animation,UploadData);

% Save workspace in current folder
save('Workspace_1D_Sphere','CD_0','CD_eq','Drug0','Drug_eq','Drug_0',...
    'Drug_CD_eq','DrugSol','DrugCDSol','MRSol','r','K','k2','D','hrs',...
